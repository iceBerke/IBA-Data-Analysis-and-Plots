...
SMOOTHING_WINDOW = 11    # Window size (must be odd number)
...
def validate_savgol_params(window, order, n_points):
    ...
    if window % 2 == 0:
        errors.append(f"Window size must be odd (currently {window})")
    
    # Check if window is smaller than data
    if window > n_points:
        errors.append(f"Window size ({window}) must be <= number of points ({n_points})")
        errors.append(f"Either increase your region size or decrease window size")
    
    # Check if polyorder is less than window
    if order >= window:
        errors.append(f"Polynomial order ({order}) must be < window size ({window})")
...
def find_max_negative_slope(x, y, region_start, region_end):
    ...
    param_errors = validate_savgol_params(SMOOTHING_WINDOW, SMOOTHING_ORDER, n_points)
    ...
    y_smooth = savgol_filter(y_region, window_length=SMOOTHING_WINDOW, polyorder=SMOOTHING_ORDER)
...
ax2a.plot(result['x_region'], result['y_smooth'], 'b-', linewidth=2.5, 
         label='Smoothed Data (Savitzky-Golay)')
