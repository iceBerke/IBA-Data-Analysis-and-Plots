...
SMOOTHING_WINDOW = 11    # Must be odd number
SMOOTHING_ORDER = 3
DERIVATIVE_SIGMA = 2     # Smoothing for derivative calculation

# ============= FUNCTIONS =============
def smooth_spectrum(y, window=11, order=3):
...
def find_max_negative_slope(x, y, region_start, region_end):
    ...
    # Smooth the data
    y_smooth = gaussian_filter1d(y_region, sigma=DERIVATIVE_SIGMA)
...
x = data[:, 0]
y = data[:, 1]
y_smooth = smooth_spectrum(y, SMOOTHING_WINDOW, SMOOTHING_ORDER)
...
result = find_max_negative_slope(x, y_smooth, REGION_START, REGION_END)
...
# ============= WINDOW 1: FULL SPECTRUM =============
fig1 = plt.figure(figsize=(14, 6))
ax1 = fig1.add_subplot(111)

ax1.plot(x, y, 'b-', linewidth=1, label='Full Spectrum')
...
ax1.set_title(f"{filename} - Full Spectrum Overview", fontsize=14, fontweight='bold')
ax1.legend(loc='best', fontsize=10)
ax1.grid(True, alpha=0.3)

# ============= WINDOW 2: ZOOMED REGION ANALYSIS =============
fig2 = plt.figure(figsize=(14, 10))

# Top subplot: Smoothed spectrum with edge
